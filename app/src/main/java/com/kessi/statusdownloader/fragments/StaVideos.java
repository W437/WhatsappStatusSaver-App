...
import com.kessi.statusdownloader.adapter.StaVideoAdapter;
...
public class StaVideos extends Fragment implements StaVideoAdapter.OnCheckboxListener {

	ArrayList<StatusModel> fileList = new ArrayList<>();
	GridView gridView;
	StaVideoAdapter adapter;
	int SAVE = 10;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		gridView = (GridView) rootView.findViewById(R.id.videoGrid);
		populateVideo();
	...
	class deleteAll extends AsyncTask<Void, Void,Void>{
		...
		protected Void doInBackground(Void... voids) {
			...
			for (int i = 0; i < filesToDelete.size(); i++) {
				StatusModel details = fileList.get(i);
	...
	class loadDataAsync extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void unused) {
			...
			new Handler().postDelayed(() -> {
				if (getActivity() != null) {
					if (fileList != null) {
						adapter = new StaVideoAdapter(StaVideos.this, fileList, StaVideos.this);
	...
	public void updateSongList() {
		File videoFiles = new File(Environment
				.getExternalStorageDirectory().toString() + "/Download/" + getResources().getString(R.string.app_name) +"/Videos");
